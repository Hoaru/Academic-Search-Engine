...
class Creat_module():
    ...
    def create_LDA(self):
        ...
        import gensim
        # 做映射，相当于词袋
        dictionary = corpora.Dictionary(self.contents_clean)  ##格式要求：list of list形式，分词好的的整个语料
        corpus = [dictionary.doc2bow(sentence) for sentence in self.contents_clean]  # 语料
...
def Output(SnowNLP_ob, Module):

    for word in SnowNLP_ob.score:#题目、摘要、评分、分类
        no = word[0]#文档编号
        kind = Module.predict(SnowNLP_ob.corpus[int(word[0])])
        score = word[1]
        ...
        print("文档编号：", type(no), no)
        print("分类:", type(kind), kind)
        print("评分：", type(score), score)
        print("标题：", type(title), title)
        print("摘要：", type(summary), summary)
        print()
...
if __name__ == '__main__':
    ...
    corpus = HMM_ob.loadArticle('ir_corpus_1000_shuffled_new.txt')
    WordSegmentation_result = HMM_ob.word_partition(corpus)
    InvertedIndex = Inverted_Index(WordSegmentation_result) #倒排索引结果
    TFIDF_ob = TFIDF(InvertedIndex.documents, InvertedIndex.Inverted_Index, ['水']) #tfidf打分结果
    ...
    Module_ob = Creat_module(Drop_stopword(WordSegmentation_result, "stoplist_utf8.txt").contents_clean, df_news)
    Output(SNOWNLP_ob, Module_ob)
